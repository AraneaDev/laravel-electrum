<template>
  <div class="electrum">
    ...
    <bootstrap-modal ref="txDetails" :needHeader="false" :needFooter="false" size="medium" v-if="transaction.txid">
      <div slot="body">
    ...
    <bootstrap-modal ref="requestDetails" :needHeader="false" :needFooter="false" size="medium" v-if="request.address">
      <div slot="body">
        <table class="table table-details table-condensed table-striped">
          <tbody>
          <tr>
          ...
          <tr>
            <th class="no-wrap">Status:&nbsp;</th>
            <td class="remaining">{{ request.status_str }}</td>
            <td class="qr-cell" rowspan="6">
              <div class="qr-code img-thumbnail img-responsive">
          ...
          <tr>
            ...
            <td class="remaining">{{ request.message ? request.message : 'none' }}</td>
          </tr>
          <tr>
            <th class="no-wrap">Date:&nbsp;</th>
            <td class="remaining">{{ new Date(request.timestamp * 1000).toLocaleString() }}</td>
          </tr>
          <tr>
...
export default {
  ...
  computed: {
    ...
    expires: function () {
      return moment.unix(this.request.timestamp + this.request.expiration).fromNow();
    },
